...
# #         grid_search = GridSearchCV(KNeighborsClassifier(), param_grid, cv=5, n_jobs=-1, scoring='loss', return_train_score=True)
...
# #     train_ds = pd.read_csv(train_file, sep=',')
# #     test_ds = pd.read_csv(test_file, sep=',')

# #     X_train, y_train, X_test, y_test = train_ds.iloc[:, 2:-1], train_ds.iloc[:, -1], test_ds.iloc[:, 2:-1], test_ds.iloc[:, -1]



# #     #best_parameters_knn = KNN.grid_search(X_train, y_train)
# #     best_parameters_svm = SVM.grid_search(X_train, y_train)

# #     SVM = SVM(**best_parameters_svm)
